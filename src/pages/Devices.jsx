...
import {
  Plus, Search, Grid3X3, List, Smartphone, Clock, Eye, Edit3, Trash2, AlertCircle,
  Upload, Camera, X, Laptop, Tablet,
} from 'lucide-react';
...
export default function Devices() {
  ...
  const [formData, setFormData] = useState({ brand: '', model: '', type: 'Smartphone', status: 'Available', image: null });
  const fileInputRef = useRef(null);

  const statuses = ['All', 'Available', 'In Service'];
  const types = ['Smartphone', 'Laptop', 'Tablet'];
    ...
    >
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
        {['Available', 'In Service'].map(status => {
          ...
          return (
            <div key={status} className="bg-white rounded-xl p-4 border border-slate-100 shadow-sm flex items-center gap-3">
              ...
              <div>
                <p className="text-xs text-slate-500">{status === 'Available' ? 'Tersedia' : 'Dalam Service'}</p>
        ...
        <div className="bg-white rounded-xl p-4 border border-slate-100 shadow-sm flex items-center gap-3">
          ...
          <div>
            <p className="text-xs text-slate-500">Total Perangkat</p>
            <p className="font-bold text-slate-900">{devices.length}</p>
          </div>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
          <AnimatePresence>
            {filteredDevices.map((item, idx) => (
              <motion.div key={item.id} initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} transition={{ delay: idx * 0.03 }} className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden hover:shadow-lg transition-all group">
                <div className="relative h-40 bg-slate-50 flex items-center justify-center p-4 overflow-hidden">
                  {item.image ? (
                    ...
                    <div className="w-20 h-20 rounded-2xl bg-gradient-to-br from-slate-200 to-slate-300 flex items-center justify-center group-hover:scale-110 transition-transform duration-500">
                      <DeviceIcon type={item.type} size={40} className="text-slate-500" />
                    </div>
                  )}
                ...
                <div className="p-5">
                  ...
                  <h4 className="font-bold text-slate-900 text-sm leading-tight mb-2">{item.model}</h4>
                  <span className="inline-flex items-center gap-1 px-2 py-1 bg-slate-100 rounded-lg text-xs font-medium text-slate-700 mb-4">{item.type}</span>
                  <div className="flex items-center justify-between pt-4 border-t border-slate-100">
                    <div className="flex items-center gap-1.5 px-2 py-1 bg-slate-100 rounded-lg">
                      <Clock size={12} className="text-slate-500" />
                      <span className="text-xs font-medium text-slate-700">{formatRelativeTime(item.createdAt)}</span>
                    </div>
            ...
            <table className="w-full text-sm">
              <thead className="bg-slate-50/80 border-b border-slate-100">
                <tr>
                  ...
                  <th className="text-left px-6 py-3.5 font-semibold text-slate-600">Tipe</th>
                  <th className="text-left px-6 py-3.5 font-semibold text-slate-600">Status</th>
                  <th className="text-left px-6 py-3.5 font-semibold text-slate-600">Ditambahkan</th>
                  ...
                  <tr key={item.id} className="hover:bg-slate-50/50 transition">
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        {item.image ? (
                          <img src={item.image} alt={item.model} className="w-10 h-10 rounded-lg object-cover bg-slate-100" />
                        ) : (
                          <div className="w-10 h-10 rounded-lg bg-slate-100 flex items-center justify-center"><DeviceIcon type={item.type} size={20} className="text-slate-500" /></div>
                        )}
                    ...
                    <td className="px-6 py-4 text-xs text-slate-500">{formatRelativeTime(item.createdAt)}</td>
                    <td className="px-6 py-4">
                      <div className="flex gap-1">
      ...
      {filteredDevices.length === 0 && (
        <div className="text-center py-12 bg-white rounded-2xl border border-slate-100">
          ...
          <p className="text-slate-500 font-medium">Tidak ada perangkat yang sesuai.</p>
        </div>
      )}
      ...
      <Modal isOpen={isAddModalOpen} onClose={() => setIsAddModalOpen(false)} title="Tambah Perangkat Baru">
        <div className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-2">Gambar Perangkat</label>
            {formData.image ? (
              ...
              <div className="border-2 border-dashed border-slate-200 rounded-xl p-6 text-center">
                ...
                <p className="text-sm text-slate-500 mb-3">Unggah gambar perangkat</p>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div><label className="block text-sm font-medium text-slate-700 mb-1.5">Model *</label><input type="text" value={formData.model} onChange={(e) => setFormData({ ...formData, model: e.target.value })} placeholder="e.g. iPhone 15 Pro" className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition" /></div>
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div><label className="block text-sm font-medium text-slate-700 mb-1.5">Tipe *</label><select value={formData.type} onChange={(e) => setFormData({ ...formData, type: e.target.value })} className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition cursor-pointer">{types.map(t => <option key={t} value={t}>{t}</option>)}</select></div>
            <div><label className="block text-sm font-medium text-slate-700 mb-1.5">Status</label><select value={formData.status} onChange={(e) => setFormData({ ...formData, status: e.target.value })} className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition cursor-pointer"><option value="Available">Tersedia</option><option value="In Service">Dalam Service</option></select></div>
          </div>
          <div className="flex gap-3 pt-4">
            ...
            <button onClick={handleAddDevice} className="flex-1 px-4 py-2.5 bg-gradient-to-r from-blue-600 to-violet-600 text-white rounded-xl font-medium hover:opacity-90 transition shadow-lg shadow-blue-500/20 cursor-pointer">Tambah Perangkat</button>
          </div>
        </div>
      </Modal>
        ...
        {selectedDevice && (
          <div className="space-y-4">
            <div className="flex items-center gap-4">
              <div className="w-20 h-20 rounded-xl bg-slate-100 flex items-center justify-center p-2 shrink-0">
            ...
            <div className="grid grid-cols-2 gap-4 bg-slate-50 p-4 rounded-xl border border-slate-100">
              <div><p className="text-xs text-slate-500 mb-1">Tipe</p><p className="font-semibold text-slate-900">{selectedDevice.type}</p></div>
            ...
            <button onClick={() => setIsViewModalOpen(false)} className="w-full px-4 py-2.5 bg-slate-100 text-slate-700 rounded-xl font-medium hover:bg-slate-200 transition cursor-pointer">Tutup</button>
          </div>
        )}
      </Modal>
